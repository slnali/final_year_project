...
'''
TODO REMOVE REDUNDANT URLS AND REMOVE FROM REPORT
'''
urlpatterns = [
    ...
    url(r'^book_slot/(?P<slot>\d+:\d+)/(?P<date>[\w|\W]+\d{2}\/\d{2}\/\d{2})/(?P<pk>\d+)/(?P<event_pk>\d+)/$',views.book_meeting_slot, name='book_meeting_slot'),
